...
type ConnectionResult struct { Status string `json:"status"`; Message string `json:"message"` }
...
func (ManualConnector) Connect(context.Context, string) (*ConnectionResult, error) { return &ConnectionResult{Status: "manual", Message: "Manual holdings enabled."}, nil }
func (ManualConnector) SyncHoldings(context.Context, string) ([]HoldingInput, error) { return nil, nil }
func (ManualConnector) SyncTransactions(context.Context, string, time.Time, time.Time) ([]PortfolioTransactionInput, error) { return nil, nil }
...
type AllocationItem struct { Name string `json:"name"`; Value float64 `json:"value"`; Percent float64 `json:"percent"` }
...
type RiskFinding struct { Severity string `json:"severity"`; Title string `json:"title"`; Explanation string `json:"explanation"` }
...
func PriceHoldings(ctx context.Context, provider marketdata.Provider, holdings []models.Holding) []models.Holding {
	...
	for _, h := range holdings { symbols = append(symbols, h.Symbol) }
	quotes, _ := provider.GetQuotes(ctx, symbols)
	bySymbol := map[string]marketdata.Quote{}
	for _, q := range quotes { bySymbol[strings.ToUpper(q.Symbol)] = q }
...
func BuildSummary(holdings []models.Holding, accounts []models.BrokerageAccount) Summary {
	...
	for _, a := range accounts { accountTypes[a.ID] = a.AccountType }
	...
	if s.TotalCostBasis > 0 { s.UnrealizedGainLossPct = s.UnrealizedGainLoss / s.TotalCostBasis * 100 }
	s.TopHoldings = allocationBy(holdings, func(h models.Holding) string { return h.Symbol }, s.TotalMarketValue)
	if len(s.TopHoldings) > 5 { s.TopHoldings = s.TopHoldings[:5] }
	return s
}

func BuildAllocation(holdings []models.Holding, accounts []models.BrokerageAccount) Allocation {
	total := 0.0
	for _, h := range holdings { total += h.MarketValue }
	accountTypes := map[string]string{}
	for _, a := range accounts { accountTypes[a.ID] = a.AccountType }
	return Allocation{
		ByAccountType: allocationBy(holdings, func(h models.Holding) string { if accountTypes[h.BrokerageAccountID] == "" { return "other" }; return accountTypes[h.BrokerageAccountID] }, total),
...
func ConcentrationRisk(holdings []models.Holding, profile models.AdvisorProfile) []RiskFinding {
	...
	top5 := 0.0
	for i, item := range bySymbol { if i < 5 { top5 += item.Percent } }
	if top5 > 60 {
	...
	stockValue := 0.0
	for _, h := range holdings { if h.SecurityType == "stock" { stockValue += h.MarketValue } }
...
func allocationBy(holdings []models.Holding, key func(models.Holding) string, total float64) []AllocationItem {
	...
	for _, h := range holdings {
		name := key(h); if name == "" { name = "other" }
		m[name] += h.MarketValue
	}
	out := make([]AllocationItem, 0, len(m))
	for k, v := range m {
		p := 0.0; if total > 0 { p = math.Round(v/total*1000) / 10 }
